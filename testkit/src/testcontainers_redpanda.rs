/// copy from test [testcontainers-redpanda-rs - crates.io: Rust Package Registry](https://crates.io/crates/testcontainers-redpanda-rs)
/// copy to be able to used it with latest version testcontainers
/// use redpanda vs kafka, because it **seems** faster to start, and uses less resources.
...
impl Redpanda {
    ...
    pub fn cmd_create_topic(topic_name: &str, partitions: i32) -> ExecCommand {
        ...
        let container_ready_conditions = vec![
            ...
            WaitFor::Duration { length: std::time::Duration::from_secs(1) },
        ];
